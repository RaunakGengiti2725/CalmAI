...
import { summarizeJournalEntry, evaluateGrowth } from '../api';
...
export default function Journal({ onInteraction, log, setLog }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!entry) return;
    setLoading(true);
    const summary = await summarizeJournalEntry(entry);
    const score = await evaluateGrowth(entry);
    setLog((l) => [...l, { prompt: prompts[currentPrompt], entry, summary }]);
    setEntry('');
    setCurrentPrompt((p) => (p + 1) % prompts.length);
    setLoading(false);
    onInteraction('positive', score);
  }
